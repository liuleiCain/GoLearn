...
Slice的内存分配由Go的内存管理器（`runtime.mallocgc`）决定，遵循以下策略：

1. **逃逸分析决定分配位置**
   - **栈分配**：未发生逃逸且大小 ≤ `maxsmallsize`（通常是32KB）的数组，在栈上分配
   - **堆分配**：发生逃逸、大小超过栈限制或包含指针类型的数组，在堆上分配

2. **堆上的分级分配策略**

| 大小范围 | 分配方式 | 说明 |
|---------|---------|------|
| ≤ 8KB | mcache/mcentral | 从P本地缓存或中央列表获取 |
| 8KB ~ 32KB | mheap | 直接从堆分配，使用span class |
| > 32KB | mheap.large | 单独分配，不放入span |

3. **内存对齐规则**
   - 通过 `runtime.roundupsize` 向上取整到 size class
   - 最小分配单位为 8 字节（64位系统）
   - 例如：请求 100 字节 → 实际分配 112 字节

4. **内存复用机制**
   - 使用 `mcache` 缓存空闲内存块，避免频繁系统调用
   - 释放的内存先归还 mcentral，可能被同 size class 的对象复用
   - GC 扫描后真正释放的内存可重用

5. **零分配优化（Zero-Copy）**
   ```go
   // 切片表达式可能不分配新内存
   data := make([]byte, 100)  // 分配新内存
   sub := data[20:50]         // 共享底层数组，零分配
   ```
